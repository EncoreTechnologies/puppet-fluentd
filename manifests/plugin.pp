# @summary `fluentd::plugin` is a Puppet class that manages the installation of Fluentd plugins.
...
# @param plugin_install_options
#   A hash of options to pass to the package provider when installing the plugin.
# @param plugin_source
#   The source URL from where the plugin will be downloaded. Default is 'https://rubygems.org'.
# @param plugin_provider
#   The provider that will be used to install the plugin. Default is 'tdagent'.
# @param repo_version
#   The version of the repository where the plugin is located. Default is '5'.
#
# @example
...
define fluentd::plugin (
  ...
  # The value of `$title` is determined by the name you give to the `fluentd::plugin`
